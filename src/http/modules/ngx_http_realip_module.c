...
ngx_http_realip_init(ngx_conf_t *cf)
{
    ...
    /*
        phases数组中有11个成员，取出NGX_HTTP_POST_READ_PHASES阶段的handlers动态数组，向其中添加ngx_http_handler_pt处理方法，这样ngx_http_realip_module模块
        就会介入HTTP请求的NGX_HTTP_POST_READ_PHASE处理阶段了
    */
    h = ngx_array_push(&cmcf->phases[NGX_HTTP_POST_READ_PHASE].handlers);
    if (h == NULL) {
        return NGX_ERROR;
    }
    //ngx_http_realip_handler方法就是实现ngx_http_handler_pt接口的方法
    *h = ngx_http_realip_handler;

    /*
        实际上，同一个HTTP模块的同一个ngx_http_realip_headler方法，完全可以设置到不同的阶段中。例如，phases[NGX_HTTP_PREACCESS_PHASE.handlers]动态数组中也添加了
